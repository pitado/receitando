...
for (const size of sizes) {
  test(`header UX depois — ${size.name}px`, async ({ page }, testInfo) => {
...
test("busca global abre por Ctrl+K e separa receitas de ingredientes", async ({ page }) => {
  ...
  await installApiMock(page, {
    ...
    "GET /api/ingredients": async () => ({
      body: [{ id: "banana", name: "Banana", normalizedName: "banana", category: "Frutas" }],
    }),
    "GET /api/v2/recipes": async () => ({
      body: {
        items: [{
          ...
          difficulty: "FACIL",
          mealType: "Sobremesa",
          source: { name: "Comunidade", externalSource: null },
        }],

    ...
    <form className={styles.form} onSubmit={onSubmit} ref={formRef}>
      <div aria-label={`Etapa ${step} de 3`} className={styles.progress}>
        <div className={styles.progressHeader}>
          ...
          <strong>{step === 1 ? "Sua receita" : step === 2 ? "Como preparar" : "Foto e revisão"}</strong>
        </div>
        <div className={styles.progressTrack}><span style={{ width: `${(step / 3) * 100}%` }} /></div>
        <ol>
          <li className={step >= 1 ? styles.stepActive : undefined}>Receita</li>
          <li className={step >= 2 ? styles.stepActive : undefined}>Preparo</li>
          <li className={step >= 3 ? styles.stepActive : undefined}>Revisão</li>
        </ol>
      </div>
      ...
      <section className={styles.stepPanel} data-step="1" hidden={step !== 1}>
        <div className={styles.stepIntro}>
          <span>01</span>
          <div><h2>Conte a história do prato</h2><p>Comece pelo essencial. Depois a gente entra nos ingredientes e no preparo.</p></div>
        </div>

        <div className={styles.gridTwo}>
          <label><span>Seu nome</span><input name="authorName" required minLength={2} placeholder="Como quer aparecer" /></label>
          <label><span>E-mail <small>(opcional)</small></span><input name="authorEmail" type="email" inputMode="email" autoComplete="email" placeholder="Para contato sobre a receita" /></label>
        </div>
        ...
        <div className={styles.gridThree}>
          <label><span>Tempo <small>(minutos)</small></span><input name="prepMinutes" type="number" min={1} max={1440} inputMode="numeric" /></label>
          <label><span>Porções</span><input name="servings" type="number" min={1} max={100} inputMode="numeric" /></label>
          <label><span>Dificuldade</span><select defaultValue="FACIL" name="difficulty"><option value="FACIL">Fácil</option><option value="MEDIA">Média</option><option value="DIFICIL">Difícil</option></select></label>
        </div>
      ...
      <section className={styles.stepPanel} data-step="2" hidden={step !== 2}>
        <div className={styles.stepIntro}>
          <span>02</span>
          <div><h2>Agora, mão na massa</h2><p>Um ingrediente e um passo por linha deixam a receita fácil de revisar e seguir.</p></div>
        </div>

        <div className={styles.gridTwo}>
          <label><span>Ingredientes</span><textarea name="ingredients" required rows={11} placeholder={"1 ingrediente por linha\n2 bananas maduras\n2 ovos\n1 xícara de farinha"} /></label>
          <label><span>Modo de preparo</span><textarea name="instructions" required rows={11} placeholder={"1 passo por linha\nAmasse as bananas\nMisture os ovos\nLeve ao forno..."} /></label>
        </div>
      </section>
      ...
      <section className={styles.stepPanel} data-step="3" hidden={step !== 3}>
        <div className={styles.stepIntro}>
          <span>03</span>
          <div><h2>Último olhar antes de enviar</h2><p>Escolha a foto e confira o resumo. Você ainda pode voltar e ajustar qualquer etapa.</p></div>
        </div>

        <div className={styles.reviewCard}>
          <div>
            <span>{review?.mealType}</span>
            <h3>{review?.title || "Sua receita"}</h3>
            <p>{review?.description}</p>
          </div>
          <dl>
            {review?.prepMinutes ? <div><dt>Tempo</dt><dd>{review.prepMinutes} min</dd></div> : null}
            {review?.servings ? <div><dt>Porções</dt><dd>{review.servings}</dd></div> : null}
            <div><dt>Dificuldade</dt><dd>{difficultyLabel}</dd></div>
            <div><dt>Conteúdo</dt><dd>{review?.ingredientsCount ?? 0} ingredientes · {review?.instructionsCount ?? 0} passos</dd></div>
          </dl>
        </div>

        <div className={styles.photoField}>
          <div className={styles.photoHeading}>
            <div><strong>Foto do prato</strong><p>No celular, toque abaixo para tirar uma foto ou escolher da galeria.</p></div>
            <span>JPG, PNG ou WebP · até 12 MB</span>
          </div>
          <label className={styles.photoPicker}>
            <input accept="image/jpeg,image/png,image/webp" name="image" onChange={handleImageChange} required type="file" />
          ...
          {previewUrl ? <div className={styles.photoPreview}><img alt="Prévia da foto escolhida" src={previewUrl} /><span>Prévia da foto que será enviada</span></div> : null}
        </div>
      ...
      <div className={styles.actions}>
        {step > 1 ? <button className={styles.back} disabled={sending} onClick={previousStep} type="button">← Voltar</button> : <span />}
        {step < 3 ? <button className={styles.next} onClick={nextStep} type="button">Continuar <span aria-hidden="true">→</span></button> : <button className={styles.submit} disabled={sending} type="submit">{sending ? "Enviando receita e foto..." : "Enviar minha receita"}</button>}
      </div>
    </form>
  );
}
